...
- Application, Electron main/preload, authorization, prompt-generator, translation, smart-reply, cache, lock-screen, remote-guard, multi-profile, and packaging source.
...
- The complete ordered eight-patch `maoyi` Signal series, manifests, checksums, and replay/build tooling.
- 应用、Electron 主进程与预加载、授权、提示词生成器、翻译、智能回复、缓存、锁屏、御前侍卫、多开和打包源码。
- WhatsApp、Telegram 和 Signal 集成源码及自动化契约测试。
- 固定的 Signal Desktop 8.18.0 官方源码归档，以及完整的8个补丁、清单、校验值和重放构建工具。
...
```text
Path: external encrypted suite archive (not stored in GitHub)
...
```text
dceadd3a1c97e40b62cc165c82a97f8efcd3f82b
```
