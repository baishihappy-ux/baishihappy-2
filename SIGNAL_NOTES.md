...
- For the current display bug, mirror the temporary always-on-top show sequence before considering heavier native `SetParent` work.
...
- Follow the competitor-style top-level window overlay model:
  ...
  - The main app continuously knows the workspace rectangle and sends it through the Signal WebSocket control channel.
