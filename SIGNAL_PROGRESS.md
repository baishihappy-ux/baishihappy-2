...
- 本机可用 Signal Desktop 路径：`<local-signal-install>\Signal.exe`。
- 友商内置 Signal 路径：`<external-reference-root>\signal\Signal.exe`。
...
- 单次启动测试可看到 `Signal.exe` 命令行包含独立 `--user-data-dir=<project-root>\.tmp\signal-launch-test`。
...
- 双实例测试通过：同时存在 2 个 `Signal.exe` 进程，分别使用：
  - `<project-root>\.tmp\signal-multi-a`
  - `<project-root>\.tmp\signal-multi-b`
...
- PowerShell `Start-Process` 测试时，如果直接传 `--user-data-dir=<workspace-root>\...`，Signal 日志会显示 `userData: D:\DF`，路径被空格截断。
